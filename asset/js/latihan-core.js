...
(function (global) {
  ...
  var Latihan = {
    ...
    openLevel: function (i) {
      this.current = i;
      this.round = 0;
      this.urutan = {};        // urutan soal dikocok ulang tiap level dibuka
      this.renderLevelBar();
      this.buildRound();
    },

    /* Ambil soal dari bank secara acak TAPI tanpa ulangan.
       Cara kerjanya: indeks bank dikocok sekali saat level dibuka, lalu
       dijalani berurutan. Jadi tiap kali level dibuka anak dapat susunan
       soal yang beda, dan dalam satu level tidak ada soal yang muncul dua
       kali. Kalau jumlah soal per level lebih banyak daripada isi bank,
       banknya dikocok ulang untuk putaran berikutnya — dengan syarat soal
       pertama putaran baru bukan soal terakhir putaran sebelumnya, supaya
       tidak terasa nyambung dua kali.

       Dipanggil dari level lewat api.soal(BANK). Aman dipanggil berkali-
       kali dalam satu ronde: hasilnya selalu sama untuk ronde yang sama. */
    ambilSoal: function (bank) {
      if (!this.urutan) this.urutan = {};
      var kunci = this.current + ':' + bank.length;
      var putaran = Math.floor(this.round / bank.length);
      var simpan = this.urutan[kunci];
      if (!simpan || simpan.putaran !== putaran) {
        var idx = [];
        for (var i = 0; i < bank.length; i++) idx.push(i);
        idx = shuffle(idx);
        if (simpan && bank.length > 1) {
          var terakhir = simpan.idx[bank.length - 1];
          if (idx[0] === terakhir) {          // tukar dengan yang tengah
            var j = Math.floor(bank.length / 2);
            idx[0] = idx[j]; idx[j] = terakhir;
          }
        }
        simpan = { putaran: putaran, idx: idx };
        this.urutan[kunci] = simpan;
      }
      return bank[simpan.idx[this.round % bank.length]];
    },
    ...
    buildRound: function () {
      ...
      var api = {
        round: this.round,
        rounds: lv.rounds,
        /* soal(BANK) -> satu soal, acak & tanpa ulangan. Jangan pakai
           BANK[api.round % BANK.length]: urutannya jadi sama terus dan
           soal di belakang bank tidak pernah kepakai. */
        soal: function (bank) { return self.ambilSoal(bank); },
